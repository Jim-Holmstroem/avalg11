...
\subsection{\proc{Nearest-Neighbour},\proc{2-opt} and \proc{2.5-opt}}
Uses \proc{Nearest-Neighbour} a full \proc{2-opt} and \proc{3-opt} with only the first permutation, here called \proc{2.5-opt}. 
The highest threshold found for \proc{2.5-opt} not resulting in timeout had a score of $21.4091$

\subsection{\proc{Nearest-Neighbour},\proc{2-opt} and \proc{3-opt}}
Uses \proc{Nearest-Neighbour} a full \proc{2-opt} and as many \proc{3-opt} with both permutations as possible.
